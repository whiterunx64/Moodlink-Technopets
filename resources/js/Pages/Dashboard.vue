        ...
        <Transition enter-active-class="transition-opacity duration-300" enter-from-class="opacity-0"
            ...
            <div v-else class="space-y-5">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-text-muted text-xs font-medium">Showing:</span>
                    <div class="flex gap-1 bg-gray-100 p-1">
                        <button v-for="p in STAT_PERIODS" :key="p.key" type="button" :class="[
                    ...
                    <span class="text-text-muted text-xs italic">Users always shows current totals</span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 xl:grid-cols-4">
                    <StatCard v-for="card in statCards" :key="card.label" :label="card.label" :value="card.value"
                        :icon="card.icon" :icon-bg="card.iconBg" :icon-color="card.iconColor" :card-bg="card.cardBg"
                        :border-color="card.borderColor" :hover-ring="card.hoverRing" :href="card.href"
                        :rows="card.rows" />
                </div>
                ...
                <div class="grid grid-cols-1 gap-4 xl:grid-cols-[3fr_2fr]">
                    ...
                    <div class="border-border-light flex flex-col border bg-white shadow-sm">
                        <!-- Header + tabs -->
                        <div class="border-border-light shrink-0 border-b px-5 pt-5 pb-0">
                            ...
                            <div class="mt-3 flex" role="tablist">
                                <button v-for="tab in ACTIVITY_TABS" :key="tab.key" type="button" role="tab"
                                    :aria-selected="activity_tab === tab.key" :class="[
                                        'relative mr-5 pb-3 text-sm font-medium transition-colors duration-150',
                        ...
                        <!-- Tab content -->
                        <div class="divide-border-light divide-y overflow-y-auto" style="height: 360px">
                            <!-- MoodSpace feed -->
                                        ...
                                        <div class="flex items-center justify-between gap-2">
                                            <div class="min-w-0">
                                                <span class="text-text-primary truncate text-sm font-medium">{{
                                                    entry.name }}</span>
                                                <span class="text-text-muted ml-1.5 text-xs">({{
                                                    entry.anonymous_name
                                                }})</span>
                                            </div>
                                            <div class="flex shrink-0 items-center gap-1.5">
                                                <span v-if="entry.mood" class="text-sm leading-none">{{
                                                    moodEmoji[entry.mood] ??
                                                    ''
                                                }}</span>
                                                <span class="text-text-muted text-xs">{{ entry.time }}</span>
                                            </div>
                                        </div>
                                        <p class="text-text-muted mt-0.5 line-clamp-1 text-xs">
                                            {{ entry.message }}
                                        </p>
                                    </div>
                                    <span :class="[
                            ...
                            <!-- Appointments -->
                            <template v-else-if="activity_tab === 'appointments'">
                            ...
                            <!-- Flagged posts -->
                            <template v-else>
                                <div v-for="entry in flagged_mood_entries" :key="entry.id"
                                    class="hover:bg-hover-soft flex items-start gap-3 px-5 py-3.5 transition-colors">
                                    <div
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex items-center justify-between gap-2">
                                            ...
                                            <span class="text-text-muted shrink-0 text-xs">{{ entry.time }}</span>
                                        </div>
                                        <p class="text-text-muted mt-0.5 line-clamp-1 text-xs">
                                            {{ entry.message }}
                                        </p>
                                ...
                                <div v-if="flagged_mood_entries.length === 0"
                                    class="text-text-muted flex flex-col items-center justify-center gap-2 py-10 text-sm">
                    ...
                    <!-- Mood trend -->
                    <MoodTrends :data="mood_trends" :stat-period="stat_period" />
                </div>
            </div>
        </Transition>
    </AdminLayout>
