...
const PRESETS = [
  ...
  { label: 'Afternoon', times: ['13:00', '14:00', '15:00', '16:00', '17:00'] },
];
...
function applyPreset(times: string[]) {
  ...
  if (allSelected) {
    ...
    form.start_times = TIME_SLOTS.map((s) => s.value).filter((v) => merged.has(v));
  }
}
      ...
      <div class="relative w-full max-w-sm rounded-2xl bg-white p-7 shadow-xl">
        ...
        <form class="space-y-4" @submit.prevent="submit">
          ...
          <div>
            <label class="text-text-secondary mb-2 block text-xs font-medium">
              Start Times
              <span v-if="selectedCount > 0" class="ml-2 font-semibold text-sidebar">
            ...
            <p v-if="form.errors.start_times" class="mt-1 text-xs text-red-500">
          ...
          <div class="flex items-center gap-3 pt-2">
            ...
            <button type="submit" :disabled="form.processing || selectedCount === 0 || !form.date"
              ...
              <span v-else-if="selectedCount === 0">Save Slots</span>
              <span v-else>Save {{ selectedCount }} {{ selectedCount === 1 ? 'Slot' : 'Slots' }}</span>
            </button>
          </div>
        </form>
      </div>
