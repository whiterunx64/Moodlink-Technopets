...
<template>
    <div class="flex items-center gap-1.5 flex-wrap sm:gap-2">
        <button v-for="tab in STUDENT_TABS" :key="tab.value" type="button"
            @click="$emit('update:modelValue', tab.value)" :class="[
            ...
            <component :is="TAB_ICONS[tab.value]" class="h-3.5 w-3.5 shrink-0 sm:h-4 sm:w-4" />
            <span>{{ tab.label }}</span>
            <span :class="[
                'rounded-full px-1.5 py-0.5 text-[10px] font-bold sm:text-xs',
                modelValue === tab.value ? TAB_COUNT[tab.value] : TAB_COUNT_IDLE[tab.value],
            ]">
                {{ counts[tab.value] ?? 0 }}
            </span>
        </button>
